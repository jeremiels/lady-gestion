...
const cardsOf = (el: WeekStrip) => [...el.renderRoot.querySelectorAll('day-card')];
const buttonsOf = (el: WeekStrip) => [...el.renderRoot.querySelectorAll<HTMLButtonElement>('.week__day')];
const sheetOf = (el: WeekStrip) => el.renderRoot.querySelector('activity-sheet');
...
describe('week-strip', () => {
  ...
  it('shows a user’s own activity by the label it stores', async () => {
    ...
    await db.events.add(
      makeEvent({ id: 'work', type: 'travail', date: wednesday, activity: 'Carrière' }),
    );

    const el = await mount();
    await waitFor(el, () => cardsOf(el).some((card) => card.activity));

    expect(cardsOf(el).find((card) => card.date === wednesday)?.activity).toBe('Carrière');
  });
  ...
  it('announces which day is expanded, and only that one', async () => {
    ...
    expect(buttonsOf(el).every((button) => button.getAttribute('aria-expanded') === 'false')).toBe(
      true,
    );
    ...
    expect(
      buttonsOf(el).map((button) => button.getAttribute('aria-expanded') === 'true'),
  ...
  it('keeps the sheet mounted after it closes, so it can animate out', async () => {
    ...
    sheetOf(el)!.dispatchEvent(new CustomEvent('sheet-close', { bubbles: true, composed: true }));
    await settled(el);
