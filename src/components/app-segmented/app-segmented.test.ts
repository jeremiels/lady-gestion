...
const mount = (value: string) =>
  fixture<AppSegmented>(
    html`<app-segmented label="Période" .options=${OPTIONS} value=${value}></app-segmented>`,
  );
...
const anchored = (el: AppSegmented) =>
  options(el).filter((option) => option.classList.contains('sliding-selection__active'));
...
describe('app-segmented sliding selection', () => {
  it('anchors the pill to exactly one segment, and to the checked one', async () => {
    ...
    expect(anchored(el).map((option) => option.textContent?.trim())).toEqual(['Mois']);
    ...
    expect(anchored(el).map((option) => option.textContent?.trim())).toEqual(['Année']);
  });
