...
describe("day-card", () => {
  ...
  it("carries the machine-readable date, whatever the display format", async () => {
    ...
    expect(el.renderRoot.querySelector("time")?.getAttribute("datetime")).toBe(
      MONDAY,
    );
  });
  ...
  it("marks today, and only when told to", async () => {
    ...
    expect(
      plain.renderRoot.querySelector("time")?.hasAttribute("aria-current"),
    ).toBe(false);
    ...
    expect(
      today.renderRoot.querySelector("time")?.getAttribute("aria-current"),
    ).toBe("date");
  });
});
