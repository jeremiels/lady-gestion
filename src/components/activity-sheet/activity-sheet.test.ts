...
import { HORSE_ID, makeEvent, makeHorse, resetDb } from '../../data/__tests__/factories.ts';
...
const chipsOf = (el: ActivitySheet) => [...el.renderRoot.querySelectorAll<AppChip>('app-chip')];
...
const submitLabel = async (el: ActivitySheet, value: string) => {
  ...
  await waitFor(el, () => el.open === false || el.renderRoot.querySelector('.activity-sheet__error') !== null);
};
...
describe('activity-sheet', () => {
  it('titles itself with the day it is about', async () => {
    ...
    expect(el.renderRoot.querySelector('app-bottom-sheet')?.heading).toBe('Lundi 15 juin');
  ...
  it('marks the chip the day already carries', async () => {
    const el = await ready(
      await mount({ existing: makeEvent({ type: 'travail', date: DATE, activity: 'longe' }) }),
    );

    expect(chipsOf(el).filter((chip) => chip.selected).map((chip) => chip.label)).toEqual(['Longe']);
  });
  ...
  it('writes the session and closes on a single tap', async () => {
    ...
    expect(await sessionOn(DATE)).toMatchObject({ activity: 'trotting', title: 'Trotting' });
  });
  ...
  it('adds a typed activity to the catalogue and applies it at once', async () => {
    ...
    expect((await activitiesRepo.listByHorse(HORSE_ID)).map((item) => item.label)).toEqual([
      'Carrière',
    ]);
    expect(await sessionOn(DATE)).toMatchObject({ activity: 'Carrière', title: 'Carrière' });
  ...
  it('refuses a blank label instead of writing an unnamed activity', async () => {
    ...
    expect(el.renderRoot.querySelector('.activity-sheet__error')?.textContent).toContain('requis');
  ...
  it('fires sheet-close so its owner can clear the day', async () => {
    const el = await ready(await mount());
    let fired = 0;
    el.addEventListener('sheet-close', () => { fired += 1; });
