 ...
 */
const horseImageUrl = new URL('../../assets/lea-ladympala-trop-mignonnes.webp', import.meta.url).href;
...
export class HorseCard extends BaseElement {
  ...
  @property({ type: String, attribute: 'context-type', reflect: true }) contextType: HorseCardContext =
    'default';
  ...
  static componentStyles = css`
    ...
    :host([context-type="horse-view"]) .horse-card__info {
      background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, var(--color-page) 100%);
      ...
      <div class="horse-card__info">
        ...
        <div class="horse-card__info-left">
          ...
          <div class="horse-card__info-meta horse-card__text-small">
            <span class="horse-card__text-small">${HORSE_SEX_LABEL[horse.sex]}</span>•
            <span class="horse-card__text-small">${formatAge(ageInYears(horse.birthDate))}</span>
            ${horse.breed ? html`•<span class="horse-card__text-small">${horse.breed}</span>` : nothing}
          </div>
          ${detailed && horse.sireNumber ? html`
            <div class="horse-card__text-small">N° SIRE : ${horse.sireNumber}</div>
          ` : nothing}
        </div>
        ${detailed ? html`
    ...
    return html`
      <section>
        ${detailed
          ...
          : html`<a href="${appHref(`/horse/${horse.id}`)}" class="horse-card pressable">${content}</a>`}
      </section>
    `;
  }
}
