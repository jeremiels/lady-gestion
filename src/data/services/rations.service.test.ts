...
const seed = async (over: Partial<RationItem>[]): Promise<RationItem[]> => {
  await db.rationItems.bulkAdd(over.map((item, index) => makeRation({ sortOrder: index, ...item })));
  return rationsRepo.listByHorse(HORSE_ID);
};

/** The sheet as submitted: a quantity per line, and the ticked checkboxes. */
const submitted = (entries: { id: string; quantity: string; seasonal?: boolean }[]): FormData => {
...
describe('field names', () => {
  it('are what the markup and the schema both read', async () => {
    ...
    expect(rationFieldNames('r1')).toEqual({ quantity: 'quantity-r1', seasonal: 'seasonal-r1' });
  });
});
...
describe('writing', () => {
  it('saves a changed quantity', async () => {
    ...
    const result = await saveRationSheet([ration!], submitted([{ id: 'fib', quantity: '2' }]));
  ...
  it('accepts a comma decimal — the separator a French keyboard gives', async () => {
    ...
    await saveRationSheet([ration!], submitted([{ id: 'fib', quantity: '1,5' }]));
  ...
  it('touches nothing at all when the sheet is submitted unchanged', async () => {
    ...
    const result = await saveRationSheet([...rations], submitted([{ id: 'fib', quantity: '1.5' }]));
...
describe('seasonality', () => {
  ...
  it('unticking clears the window', async () => {
    const [ration] = await seed([{ id: 'huile', quantity: 40, season: DEFAULT_SEASON }]);

    await saveRationSheet([ration!], submitted([{ id: 'huile', quantity: '40' }]));
...
describe('rejecting', () => {
  ...
  it('rejects a negative quantity', async () => {
    ...
    const result = await saveRationSheet([ration!], submitted([{ id: 'fib', quantity: '-1' }]));
  ...
  it('rejects a blank quantity rather than reading it as zero', async () => {
    ...
    const result = await saveRationSheet([ration!], submitted([{ id: 'fib', quantity: '' }]));
