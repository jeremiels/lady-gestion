...
describe('summariseSuspension', () => {
  ...
  it('names the shared window when every suspended line agrees', () => {
    expect(summariseSuspension([null, null, null, WINTER, WINTER], '2026-08-12')).toBe(
      '2 produits saisonniers suspendus d’avril à octobre.',
    );
  });
  ...
  it('falls back to a bare count when the windows disagree', () => {
    expect(summariseSuspension([WINTER, { from: 11, to: 3 }], '2026-08-12')).toBe(
      '2 produits saisonniers suspendus.',
    );
  });
