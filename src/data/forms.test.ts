...
const expectValue = <T>(result: ParseResult<T>): T => {
  if (!result.ok) throw new Error(`expected a parsed value, got: ${result.error}`);
  return result.value;
};
...
describe('required narrows the result type', () => {
  it('drops null from the parsed type when required is true', () => {
    ...
    expect([title, date, amount, type]).toEqual(['Ferrure', '2026-08-12', 1250, 'veto']);
  });
...
describe('readForm', () => {
  ...
  it('parses every field into one typed object', () => {
    const result = readForm(formData({ title: 'Ferrure', quantity: '1,5' }), schema);
  ...
  it('reports every failure at once, not just the first', () => {
    ...
    const result = readForm(formData({ title: '', quantity: 'beaucoup' }), schema);
  ...
  it('treats an absent field the same as a blank one', () => {
    ...
    expect(result).toMatchObject({ ok: true, value: { quantity: null, seasonal: false } });
  });
  ...
  it('accepts a FormData built from generated field names', () => {
    ...
    const ids = ['a', 'b'];
    const generated = Object.fromEntries(ids.map((id) => [`quantity-${id}`, decimal()]));
