...
describe('create', () => {
  ...
  it('falls back to a generic mime type when the blob has none', async () => {
    const created = await documentsRepo.create(
      { horseId: HORSE_ID, eventId: null, category: 'autre', name: 'scan', issuedAt: null },
      new Blob(['x']),
    );
...
describe('listByHorse', () => {
  it('sorts by issue date, newest first', async () => {
    ...
    expect(documents.map((document) => document.id)).toEqual(['new', 'mid', 'old']);
  });
  ...
  it('falls back to createdAt for a document with no printed date', async () => {
    ...
    expect(documents.map((document) => document.id)).toEqual(['undated', 'dated']);
  });
...
describe('remove', () => {
  it('soft-deletes the metadata but hard-deletes the bytes', async () => {
    const created = await documentsRepo.create(
      { horseId: HORSE_ID, eventId: null, category: 'facture', name: 'f.pdf', issuedAt: null },
      new Blob(['bytes']),
    );
    ...
    expect(await db.documents.get(created.id)).toMatchObject({ deletedAt: expect.any(String) });
