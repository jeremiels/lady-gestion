...
describe("add", () => {
  ...
  it("does not reuse the slot left by a soft-deleted line", async () => {
    ...
    await seedRations([
      { id: "gone", sortOrder: 3, deletedAt: "2026-02-01T00:00:00.000Z" },
    ]);
  ...
  it("stores a season window verbatim", async () => {
    ...
    expect((await rationsRepo.get(added.id))?.season).toEqual({
      from: 10,
      to: 4,
    });
  });
...
describe("remove", () => {
  it("soft-deletes, keeping the row as a tombstone", async () => {
    ...
    expect(await db.rationItems.get("a")).toMatchObject({
