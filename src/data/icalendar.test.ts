...
describe("occurrencesByDate", () => {
  ...
  it("still shows a zero-duration event on its start day", () => {
    ...
    expect(
      occurrencesByDate([event], "2026-01-01", "2026-01-31").get("2026-01-22"),
    ).toEqual([event]);
  });
  ...
  it("ignores events outside the range entirely", () => {
    ...
    expect(
      occurrencesByDate([before, after], "2026-01-01", "2026-01-31").size,
    ).toBe(0);
  });

  it("lists all-day entries first, then by time", () => {
    const timedLate = calendarEvent({
      uid: "late",
      startTime: "16:00",
      summary: "Cours",
    });
    const timedEarly = calendarEvent({
      uid: "early",
      startTime: "09:30",
      summary: "Ferrure",
    });
  ...
  it("breaks ties on the summary so the order is stable", () => {
    ...
    const day = occurrencesByDate([b, a], "2026-01-01", "2026-01-31").get(
      "2026-01-22",
    );
