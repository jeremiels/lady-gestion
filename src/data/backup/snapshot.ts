...
export const importBackup = async (
  ...
  const backup = assertSnapshot(snapshot);
...
const assertSnapshot = (value: unknown): BackupSnapshot => {
  ...
  // An older file is refused rather than read. The steps that brought v1..v12
  // files up to date were dropped with `db.ts`'s upgrade chain, once the one
  // install in use was on v13 — and merging an old file's rows unchanged would
  // write records this build silently misreads. A version that must still
  // restore the previous one's files relaxes this and brings the file up to
  // date in `importBackup`, before the merge.
  if (snapshot.schemaVersion < SCHEMA_VERSION) {
    throw new Error(
      `Sauvegarde créée par une version trop ancienne de l'application (schéma ${snapshot.schemaVersion} < ${SCHEMA_VERSION}).`,
    );
  }
