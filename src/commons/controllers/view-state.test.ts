...
const mount = () =>
  fixture<ViewStateTestHost>(
    html`<view-state-test-host></view-state-test-host>`,
  );
...
describe("ViewState", () => {
  ...
  it("keeps two views on one entry out of each other’s way", async () => {
    ...
    const other = new ViewState<{ granularity: string }>(
      new StubHost(),
      "budget",
      () => ({
        granularity: "month",
    ...
    expect(stored()).toEqual({
      views: {
        events: { mode: "list", day: "today" },
        budget: { granularity: "year" },
      },
    });
  ...
  it("ignores a stored bag that is not an object, and keys the defaults do not declare", async () => {
    ...
    expect((await mount()).ui.value).toEqual({
      mode: "calendar",
      day: "today",
    });
    ...
    navigation.updateCurrentEntry({
      state: { views: { events: { mode: "list", gone: true } } },
    });
