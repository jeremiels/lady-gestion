...
const switchToList = async (el: EventsView) => {
  el.querySelector('app-segmented')!.dispatchEvent(
    new CustomEvent('segment-change', { detail: { value: 'list' }, bubbles: true, composed: true }),
  );
  await settled(el);
};

const search = async (el: EventsView, text: string) => {
  const input = el.querySelector('app-input')!.renderRoot.querySelector('input')!;
  input.value = text;
...
const cardIds = (el: EventsView) => [...el.querySelectorAll('event-card')].map((card) => card.event?.id).sort();
...
describe('events-view', () => {
  ...
  it('list mode hides cancelled events and finds the rest through search, folding accents and case', async () => {
    await db.events.bulkAdd([
      makeEvent({ id: 'checkup', type: 'veto', title: 'Contrôle œil', date: todayISO() }),
      makeEvent({ id: 'shoeing', type: 'marechal', title: 'Ferrure', date: addMonths(todayISO(), -1) }),
      makeEvent({ id: 'called-off', title: 'Annulé', date: todayISO(), status: 'cancelled' }),
    ]);
  ...
  it('reopens on the list, with the chip and the search text the visit was left on', async () => {
    await db.events.bulkAdd([
      makeEvent({ id: 'checkup', type: 'veto', title: 'Contrôle œil', date: todayISO() }),
      makeEvent({ id: 'vaccine', type: 'veto', title: 'Vaccin', date: todayISO() }),
      makeEvent({ id: 'shoeing', type: 'marechal', title: 'Ferrure', date: todayISO() }),
    ]);
    ...
    await waitFor(reopened, () => reopened.querySelector('event-card') !== null);
  ...
  it('reopens on the day the calendar was left on', async () => {
    ...
    el.querySelector('app-calendar')!.dispatchEvent(
      new CustomEvent('date-select', { detail: { date: day }, bubbles: true, composed: true }),
    );
    await settled(el);
  ...
  it('keeps surviving rows as the same nodes, in order, when a filter narrows the list', async () => {
    await db.events.bulkAdd([
      makeEvent({ id: 'newest', type: 'veto', title: 'Vaccin', date: todayISO() }),
      makeEvent({ id: 'middle', type: 'marechal', title: 'Ferrure', date: addMonths(todayISO(), -1) }),
      makeEvent({ id: 'oldest', type: 'veto', title: 'Dentiste', date: addMonths(todayISO(), -2) }),
    ]);
