...
const quantityField = (el: HorseView, rationId: string) =>
  [...el.querySelectorAll('app-input')].find((input) => input.name === `quantity-${rationId}`)!;
...
describe('horse-view', () => {
  it('shows the identity card, falling back to — for what is unknown', async () => {
    ...
    const values = [...identity.querySelectorAll('.meta-value')].map((node) => node.textContent?.trim());
  ...
  it('shows the empty ration state and disables editing when there are no rations', async () => {
    ...
    expect(el.textContent).toContain('Aucune ration enregistrée pour le moment.');
    expect(el.querySelector<HTMLButtonElement>('.horse-view__edit-button')?.disabled).toBe(true);
  });
  ...
  it('writes only the ration lines that were actually changed', async () => {
    await db.rationItems.bulkAdd([
      makeRation({ id: 'ration-a', label: 'Fib & Fib', quantity: 1.5, unit: 'L', sortOrder: 0 }),
      makeRation({ id: 'ration-b', label: 'CMV Minéral', quantity: 2, unit: 'kg', sortOrder: 1 }),
    ]);
  ...
  it('ticking Saisonnier stores the default window, and unticking clears it', async () => {
    await db.rationItems.bulkAdd([
      makeRation({ id: 'ration-a', label: 'Fib & Fib', quantity: 1.5, unit: 'L', season: null, sortOrder: 0 }),
      makeRation({
        id: 'ration-b',
        label: 'Huile de lin',
        quantity: 0.1,
