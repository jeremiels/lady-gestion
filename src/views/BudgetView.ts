 ...
 * February, where `sumByCategory` simply never offers it.
 */
type BudgetUiState = {
...
export class BudgetView extends LightElement {
   ...
   * memory is correct and cheap at this size; `PostsView` does the same for
   * the same reason.
   *
   * `listBudget` already drops rows with no amount and cancelled ones.
   */
  #budget = activeHorseQuery<Post[]>(
    this,
    (horseId) => postsRepo.listBudget(horseId),
    [],
  );
  ...
  /**
   * Curried so the type travels with the handler, the way `PostsView` binds
   * its type chips.
   */
  ...
  render() {
    ...
    const slices = sumByCategory(events, types);
   ...
   */
  #donutSlices(slices: BudgetSlice[], types: ResolvedCategory[]): DonutSlice[] {
    return slices.map((slice) => {
      const type = findCategory(types, slice.type);
      return {
        id: slice.type,
        label: type?.label ?? "",
        value: slice.cents,
   ...
   */
  #renderLegend(slices: BudgetSlice[], types: ResolvedCategory[]) {
    const hidden = this.#hidden;
   ...
   * shipped catalogue.
   */
  #renderBreakdown(slice: BudgetSlice, types: ResolvedCategory[]) {
      ...
      <details class="budget-view__legend-breakdown">
        ...
        <ul>
          ${repeat(
            ...
            (child) => html`
              <li>
                <span>${findCategory(types, child.type)?.label ?? ""}</span>
