...
import { makeDocument, makeHorse, resetDb } from '../data/__tests__/factories.ts';
import { fixture, waitFor } from '../components/__tests__/fixture.ts';
import { DOCUMENT_CATEGORIES, documentCategory } from '../types/document.types.ts';
import './DocumentsView.ts';
import type { DocumentsView } from './DocumentsView.ts';

const mount = () => fixture<DocumentsView>(html`<documents-view></documents-view>`);

const folderNamed = (el: DocumentsView, label: string) =>
  [...el.querySelectorAll('app-folder')].find((folder) => folder.name === label)!;
...
describe('documents-view', () => {
  it('shows every category up front, including ones with nothing filed yet', async () => {
    ...
    expect(el.querySelectorAll('app-folder')).toHaveLength(DOCUMENT_CATEGORIES.length);
    expect([...el.querySelectorAll('app-folder')].every((folder) => folder.number === 0)).toBe(true);
  });
  ...
  it('counts documents per category, and leaves an empty one at zero', async () => {
    ...
    expect(folderNamed(el, documentCategory.label('ordonnance')).number).toBe(1);
